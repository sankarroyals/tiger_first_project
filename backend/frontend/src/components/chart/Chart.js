...
const Chart = ({ title }) => {
  ...
  // seperating keys for select drop-down
  for (const [key] of Object.entries(raw[0])) {
    keys.push(key)
  ...
  const [chartV, setChartV] = useState('line')
  const values = []
  for (let i = 0; i < 7; i++) {
    values.push(200 * i)
  }
  ...
  // placing selected x-axis values in xA array
  const xVal = (e) => {
  ...
  //placing xA,yA values as data=[[],[],[].....] for charts
  useEffect(() => {
    if (x !== '' && y !== '') {
      setData([])
      const f = []
  ...
  return (
    ...
    <div className="chartD mt-4" >
      ...
      <div className="chart" style={{ position: "relative" }}>
        ...
        {data.length === 0 && <div
          style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", height: "100%", fontSize: "25px" ,position:"absolute",marginLeft:"150px",bottom:"50px"}}
        >Select the axis type to built chart</div>}
      ...
      <div className="cB">
        <div className="selction" style={{ display: "flex", gap: "10px" }}>
